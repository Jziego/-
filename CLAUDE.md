...
| Directory | Purpose |
|-----------|---------|
| `app/api/` | Next.js API route handlers (no auth middleware yet) |
...
- Current auth model: `demoOwnerId` from `lib/runtime-store.ts` is a placeholder — **Phase 5 will add real auth**
...
- **Current state:** No auth. All routes use `demoOwnerId`. This is NOT a bug — it's intentional placeholder until Phase 5.
- **New routes MUST NOT** introduce their own ad-hoc auth — wait for the Phase 5 auth system.
- **After Phase 5 lands:** Every route MUST verify `ownerId` matches the authenticated user. Never trust a client-supplied `ownerId` directly.
...
- **Rate limiting:** Not yet implemented — consider before production.
