...
在 `worker/processors/index.ts` 注册 + `worker/index.ts` 起 Worker。独立预览路由入队的 talking_head job 没有 projectId —— step 7 的产物持久化方式见 §8 Q1（倾向：写一条 `kind:"talking_head"`、`renderProjectId` 为预览专属占位的 VideoOutput，最小改动）。
...
3. 拉输入到 worker /tmp（R2 GET → 本地文件）：
     th.mp4（若有）、选中素材（image/video）、bgm
...
- **Q1 独立预览的 talking-head 产物持久化**：无 projectId 时落哪？（§D）倾向：预览只存 R2 key + 一条 `kind:"talking_head"` 的游离 VideoOutput（renderProjectId 用预览占位），或独立 `AvatarRender` 表。**待 spec 复审时定** —— 倾向前者（最小改动）。
- **Q2 预览是否扣配额**：talking-head 调用消耗 HeyGen 额度。倾向预览也扣配额（防滥用），但用单独计数器。**待定。**
- **Q3 BGM 库来源**：`bgmTrackId` 指向何处？预设音乐放 R2 固定前缀还是 bundle 进镜像？**待实现时确认**（不影响架构）。
