...
export async function GET() {
  try {
    ...
    console.error("Failed to list store profiles:", error);
    return jsonError(error instanceof Error ? error.message : "Failed to list store profiles", 500);
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return jsonError(error instanceof Error ? error.message : "Failed to create store profile", 500);
  }
}
