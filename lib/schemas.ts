...
export const storeProfileSchema = z.object({
  ...
  mainProducts: z.array(z.string().min(1).max(30)).min(1, "至少填写一个主营产品").max(10, "主营最多 10 条"),
  averageOrderValue: z.number().positive().optional(),
  targetCustomers: z.array(z.string().min(1)).min(1, "至少填写一个目标顾客"),
  sellingPoints: z.array(z.string().min(1).max(30)).min(1, "至少填写一个卖点").max(12, "特色最多 12 条"),
